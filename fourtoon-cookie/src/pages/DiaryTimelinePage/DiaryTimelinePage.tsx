...
const DiaryTimelinePage = () => {
    ...
    const fetchDiaries = async (page: number) => {
        try {
            const response = await fetch(`http://localhost:8080/diary/timeline/1?pageNumber=${page}`); // Adjust the URL based on your actual endpoint
            ...
            setDiaries(prevData => [...prevData, ...updatedData]);
        } catch (error) {
